...
def train(generator, discriminator, init_step, loader, total_iter=600000):
    ...
    for i in pbar:
        ...
        with torch.autocast('cuda'):  # <-- ADICIONAR ESSA LINHA
            ...
            grad_penalty = 10 * ((grad_x_hat.view(grad_x_hat.size(0), -1).norm(2, dim=1) - 1)**2).mean()

            # soma das perdas do discriminator
            d_loss = fake_predict - real_predict + grad_penalty

        scaler.scale(d_loss).backward()  # <-- ALTERAÇÃO AQUI
        scaler.step(d_optimizer)         # <-- ALTERAÇÃO AQUI
        scaler.update()                  # <-- ALTERAÇÃO AQUI
        
        disc_loss_val += d_loss.item()
        grad_loss_val += grad_penalty.item()

        discriminator.zero_grad() # <--- claramente adicione esta linha
        generator.zero_grad()
        ...
        if (i + 1) % n_critic == 0:
            ...
            #with torch.cuda.amp.autocast():  # <-- ALTERAÇÃO AQUI
            with torch.autocast('cuda'):

           
                # claramente recalcula o fake_image (novo forward)
                gen_z = torch.randn(b_size, input_code_size).to(device)
                fake_image_new = generator(gen_z, step=step, alpha=alpha)
                predict = discriminator(fake_image_new, step=step, alpha=alpha)
                loss = -predict.mean()

            scaler.scale(loss).backward()  # <-- ALTERAÇÃO AQUI
            scaler.step(g_optimizer)       # <-- ALTERAÇÃO AQUI
            scaler.update()                 # <-- ALTERAÇÃO AQUI
        ...
        if (i + 1) % 1000 == 0 or i==0:
            ...
            # Liberação após salvar imagens geradas
            gc.collect()
        ...
        if (i+1) % 10000 == 0 or i==0:
            ...
            # Liberação após checkpoint
            gc.collect()
            torch.cuda.empty_cache()

        if (i+1)%5000 == 0:
            state_msg = (f'{i + 1}; G: {gen_loss_val/(5000//n_critic):.3f}; D: {disc_loss_val/5000:.3f};'
                f' Grad: {grad_loss_val/5000:.3f}; Alpha: {alpha:.3f}')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--batch_size', type=int, default=2, help='how many images to train together at one iteration')
